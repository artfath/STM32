...
void STL_TranspMarchInit(void)
{
  /* start address of the test has to be aligned to 16 address range */	  
   pRunTimeRamChk = (uint32_t *)((uint32_t)SIL_START & 0xFFFFFFFCuL);
...
/**
  ...
  * @param  : None
  * @retval : ClassBTestStatus (TEST_RUNNING, SIL_DATA_FAIL,
...
ClassBTestStatus STL_TranspMarch(void)
{
  ...
  if ((((uint32_t)pRunTimeRamChk) ^ ((uint32_t)pRunTimeRamChkInv)) == 0xFFFFFFFFuL)
  {
    if (pRunTimeRamChk >= SIL_END)
    {
      ...
      pRunTimeRamChk = SIL_START;
      pRunTimeRamChkInv = ((uint32_t *)~((uint32_t)SIL_START));
    }
    else
