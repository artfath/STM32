...
#endif /* HSE_VALUE */
#define SYSTCLK_AT_RUN_HSE (uint32_t)(400000000uL)
/* system clock when HSI is applied as PLL source (HSE is not used) */
#define SYSTCLK_AT_STARTUP (uint32_t)(400000000uL)
#define SYSTCLK_AT_RUN_HSI (uint32_t)(400000000uL)
...
#define SIL_ROWS (((uint32_t)SIL_END - (uint32_t)SIL_START) / (RT_RAM_BLOCKSIZE - (2u * RT_RAM_BLOCK_OVERLAP)))
/* +2 below is for last block & buffer self-test itself */
#define RAM_TEST_COMPLETED ((uint32_t)((uint32_t)DELTA_ISR * (uint32_t)((SIL_ROWS / 4u) + 2u)))
...
#define control_flow_resume(a) CtrlFlowCntInv -= (a)
  ...
  #define ROM_SIZEinWORDS (uint32_t) ((ROM_SIZE)/4u) // substrac 4 for place check shum
  ...
  #define RAM_BCKUP ((uint32_t *)0x38000000uL)

  /* Constants necessary for execution of transparent run time March tests */
  #define SIL_START ((uint32_t *)0x20000030uL)
  #define SIL_END   ((uint32_t *)0x2000007BuL)
