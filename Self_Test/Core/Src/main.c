...
#endif /* __GNUC__ */
#define MESSAGE1   " STM32F4xx Cortex-M4 "
/* USER CODE END PD */
...
void StartUpClock_Config(void) {
	...
	/* Activate PLL with HSI as source to obtain 168 MHz fHCLK */
	RCC_OscInitStruct.OscillatorType = RCC_OSCILLATORTYPE_HSI;
	RCC_OscInitStruct.HSIState = RCC_HSI_ON;
	RCC_OscInitStruct.PLL.PLLState = RCC_PLL_ON;
	RCC_OscInitStruct.PLL.PLLSource = RCC_PLLSOURCE_HSI;
//	RCC_OscInitStruct.HSICalibrationValue = RCC_HSICALIBRATION_DEFAULT; /* !!! DEFAULT TRIM IS FORCED !!! */
	RCC_OscInitStruct.PLL.PLLM = 8;
	RCC_OscInitStruct.PLL.PLLN = 84;
	RCC_OscInitStruct.PLL.PLLP = RCC_PLLP_DIV2;
	RCC_OscInitStruct.PLL.PLLQ = 7;
...
void send(const char *rx){
	HAL_UART_Transmit(&UartHandle, (uint8_t *) rx, strlen(rx) , 0xFFFF);
}
/* USER CODE END 4 */
